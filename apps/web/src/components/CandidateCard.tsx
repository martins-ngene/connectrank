...
export const CandidateCard: React.FC<CandidateCardProps> = ({
  ...
  return (
    <div className={`glass-card rounded-2xl p-4 sm:p-5 relative overflow-hidden flex flex-col justify-between ${
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-2.5 mb-3">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-semibold text-white text-sm sm:text-base hover:text-indigo-300 transition-colors truncate">
                {candidate.name}
              </h3>
          ...
          <div className="text-right shrink-0">
            <div className="inline-flex items-center gap-1 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs font-bold font-display">
        ...
        {/* Position Title */}
        <div className="flex items-start gap-1.5 text-xs text-slate-300 mb-3 bg-slate-900/60 p-2 sm:p-2.5 rounded-xl border border-slate-800/60">
          <Briefcase className="w-3.5 h-3.5 text-indigo-400 mt-0.5 shrink-0" />
        ...
        <div className="flex flex-wrap gap-1 sm:gap-1.5 mb-4">
          ...
          {candidate.is_remote_friendly && (
            <span
              className="text-[10px] sm:text-[11px] font-medium px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-300 border border-emerald-500/30 flex items-center gap-1"
      ...
      <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between gap-2">
        {candidate.url ? (
          <a
            ...
            className="flex items-center gap-1 text-xs text-slate-400 hover:text-indigo-300 transition-colors py-1"
          >
            <span>LinkedIn Profile</span>
            <ExternalLink className="w-3 h-3" />
          </a>
        ) : (
          <span className="text-xs text-slate-500 py-1">No profile link</span>
        )}

        <button
          onClick={() => onDraftDM(candidate)}
          className="flex items-center gap-1.5 px-3 py-2 sm:py-1.5 rounded-lg bg-indigo-600/20 hover:bg-indigo-600 text-indigo-300 hover:text-white border border-indigo-500/30 text-xs font-medium transition-all active:scale-95 cursor-pointer"
        >
