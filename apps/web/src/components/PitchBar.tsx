  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <form onSubmit={handleSubmit} className="relative">
        <div className="relative flex items-center">
          ...
          <input
            ...
            className="w-full pl-10 sm:pl-12 pr-22 sm:pr-28 py-3.5 sm:py-4 bg-slate-900/90 border border-slate-700/70 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 rounded-2xl text-white placeholder-slate-500 text-sm sm:text-base shadow-xl backdrop-blur-md outline-none transition-all"
          />
          ...
          {pitch && (
            <button
              type="button"
              onClick={() => setPitch('')}
              className="absolute right-20 sm:right-24 text-slate-400 hover:text-slate-200 p-1.5 rounded-full"
      ...
      <div className="mt-3 flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1 sm:flex-wrap text-xs">
        <span className="text-slate-400 font-medium shrink-0">Quick Pitches:</span>
        {PRESET_PITCHES.map((preset) => (
          <button
            key={preset}
            onClick={() => handleSelectPreset(preset)}
            className="shrink-0 whitespace-nowrap px-2.5 py-1 rounded-full bg-slate-800/80 hover:bg-indigo-950/60 text-slate-300 hover:text-indigo-300 border border-slate-700/60 hover:border-indigo-500/30 transition-all cursor-pointer"
          >
            {preset}
          </button>
        ))}
